...
def _get_unk_masked(phrases, mask_token, i):
  # TODO: all phrases are masked to one, configure it to return multiple mask
  return phrases[:i] + [mask_token] + phrases[i+1:]
...
def get_important_scores(
  ...
  masked_entries = []
  for i in range(len(entry['phrases'])):
    token = ''.join(entry['phrases'][i].split())
    # not in stop words, are alphanumeric, not numeric
    if token not in stop_words and token.isalnum() and not token.isdigit():
      masked_entries.append(_get_unk_masked(entry['phrases'], "[UNK]", i))


  leave_1_probs = []
  for candidate in masked_entries:
    encoded = tokenizer(candidate,
                        truncation=True,
                        padding='max_length',
    ...
    leave_1_logits = target_model(encoded['input_ids'].to(device), encoded['attention_mask'].to(device))[0].squeeze()
    leave_1_probs.append(leave_1_logits)

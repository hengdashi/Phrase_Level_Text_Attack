...
if __name__ == "__main__":
  ...
  # create the attacker
  params = {'k':15, 'beam_width':8, 'conf_thres':3.0, 'sent_semantic_thres':0.7, 'change_threshold':0.4}
  ...
  (logs_pth/"features").mkdir(parents=True, exist_ok=True)
  (logs_pth/"eval").mkdir(parents=True, exist_ok=True)
  (logs_pth/"adv_set").mkdir(parents=True, exist_ok=True)
  output_pth = f'{logs_pth}/features/features_{suffix}.json'
  eval_pth = f'{logs_pth}/eval/eval_{suffix}.json'
  adv_set_pth = f'{logs_pth}/adv_set/adv_{suffix}.json'
  ...
  with torch.no_grad():
    for i, entry in enumerate(progressbar):
      ...
      num_succeeded = sum([output_entry['success'] for output_entry in output_entries])
